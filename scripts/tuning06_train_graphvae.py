...
params = {'x_dim': 3, 'enc_hidden_channels': 128, 'z_dim': 32, 'num_enc_hidden_layers': 4,
                 ...
                 'enc_act': torch.nn.functional.silu, 'dec_act': torch.nn.LogSigmoid()}
mod = GraphVAE(**params)
mod = mod.to(device)
opt = torch.optim.Adam(mod.parameters(), lr = 1e-3, weight_decay = 1e-4)

# train model
fig_dir = '/afs/crc.nd.edu/user/c/coballe/graphvaes/figs/tuning_architectures/06/'
...
os.chdir(model_dir)
torch.save(mod.state_dict(), 'mod_128_32_silu_ls.pth')
